...
export default function BonafideAdminDashboard() {
  const { user, loading } = useAuth();
  const router = useRouter();
  const [apps, setApps] = useState<BonafideApplication[]>([]);
  ...
  useEffect(() => {
    ...
    if (user && ["HOD"].includes(user.role)) {
      fetchApps();
    } else if (user) {
  ...
  const fetchApps = async () => {
    try {
      ...
      console.error("Failed to fetch Bonafide Apps:", err);
    } finally {
      setFetching(false);
    }
  };
  ...
  return (
    <div className="min-h-screen pb-12">
      <PageHeader title="Bonafide Administration" subtitle="Manage Bonafide Certificates" />
      ...
      <div className="p-6 lg:p-8 space-y-6">
        ...
        {apps.length === 0 ? (
          <div className="bg-white border border-gray-200/60 rounded-2xl p-12 text-center shadow-sm">
            ...
            <p className="text-gray-400 text-sm mt-1">No Bonafide requests found for your role at the moment.</p>
          </div>
        ) : (
          <div className="bg-white border border-gray-200/60 rounded-2xl shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
              <h3 className="text-base font-bold text-gray-900">Bonafide Applications</h3>
            </div>
                ...
                <tbody className="divide-y divide-gray-100/80">
                  {apps.map((app) => (
                    <tr
                      ...
                      onClick={() => router.push(`/bonafide/review/${app.id}`)}
                    >
                      <td className="px-6 py-4">
                        <p className="font-medium text-sm text-gray-900">{app.studentName}</p>
                        <p className="text-xs text-gray-400 mt-0.5">{app.enrollmentNo} - Sem {app.semester}</p>
                      </td>
                      <td className="px-6 py-4">
                        <span className="text-sm font-mono bg-green-50 text-green-700 px-2.5 py-1 rounded border border-green-200">
                          {app.applicationNo}
                        </span>
                      </td>
                      <td className="px-6 py-4">
                        <span className={`text-xs px-2.5 py-1 rounded-full font-semibold border ${
                          ...
                          {app.status === "FULLY_APPROVED" ? "APPROVED" : app.status}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/bonafide/review/${app.id}`);
                          }}
