...
export default function NocReviewPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  ...
  if (!application) {
    return (
      <div className="min-h-screen pb-12">
        <PageHeader title="NOC Application" subtitle="Not Found" />
  ...
  const isTPC = user?.role === "TP_OFFICER";
  const isSuperAdmin = false; // Admin/Super Admin cannot edit results
  ...
  const renderField = (label: string, fieldKey: string, type = "text") => {
      ...
      return (
        <div className="mb-4">
          ...
          {type === "date" ? (
            <input
              type="date"
              className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg outline-none text-gray-900 focus:bg-white focus:ring-2 focus:ring-blue-500"
              value={editForm[fieldKey] ? new Date(editForm[fieldKey]).toISOString().split('T')[0] : ""}
    ...
    if (type === "date" && val) val = new Date(val).toLocaleDateString("en-IN");
  ...
  return (
    <div className="min-h-screen pb-12">
      <PageHeader title="NOC Application Details" subtitle={`Application No: ${application.applicationNo}`} />
      ...
      <div className="p-6 lg:p-8 max-w-5xl mx-auto space-y-6">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          <div className="border-b border-gray-100 bg-gray-50 px-6 py-4 flex justify-between items-center">
            ...
            {isSuperAdmin && (
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-semibold transition-colors ${isEditMode ? 'bg-gray-200 text-gray-800' : 'bg-blue-600 text-white hover:bg-blue-700'}`}
              >
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
            ...
            {renderField("Current Year", "currentYear")}
            {renderField("Company Name", "companyName")}
            {renderField("Company Location", "companyLocation")}
            {renderField("Company Address", "companyAddress")}
            {renderField("Company Website", "companyWebsite")}
            {renderField("Duration (Days)", "durationInDays", "number")}
            {renderField("Start Date", "startDate", "date")}
            {renderField("End Date", "endDate", "date")}
            {renderField("Recipient Name", "recipientName")}
            {renderField("Recipient Designation", "recipientDesignation")}
            {renderField("Applying Through", "applyingThrough")}
            {renderField("Stipend", "stipend")}
          </div>
        ...
        {application.offerLetterUrl && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 flex justify-between items-center">
            <div>
              <h4 className="text-base font-bold text-gray-900">Offer Letter / Proof</h4>
              <p className="text-sm text-gray-500 mt-1">Uploaded document by the student.</p>
            </div>
            <a href={application.offerLetterUrl} target="_blank" rel="noreferrer" className="px-4 py-2 bg-blue-50 text-blue-600 font-semibold rounded-lg hover:bg-blue-100">
              View Document
            </a>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             ...
             {isHOD && application.hodApprovalStatus === "UNDER_REVIEW" && (
                <div className="space-y-3 mt-4 border-t pt-4">
                  <textarea
                    ...
                    className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg outline-none text-sm focus:ring-2 focus:ring-blue-500"
                    value={remarks}
           ...
           <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
             <h4 className="text-base font-bold text-gray-900 mb-4 border-b pb-2">TPC Approval</h4>
             <p className="text-sm text-gray-600 mb-2">Status: 
               <span className={`ml-2 px-2 py-1 text-xs rounded font-bold ${application.tpcApprovalStatus === 'APPROVED' ? 'bg-green-100 text-green-800' : application.tpcApprovalStatus === 'REJECTED' ? 'bg-red-100 text-red-800' : 'bg-yellow-100 text-yellow-800'}`}>
                 {application.tpcApprovalStatus}
               </span>
             </p>
             <p className="text-sm text-gray-600 mb-4">Remarks: {application.tpcRemarks || "None"}</p>
             
             {isTPC && application.tpcApprovalStatus === "UNDER_REVIEW" && (
                <div className="space-y-3 mt-4 border-t pt-4">
                  <textarea
                    placeholder="Add remarks (optional)"
                    className="w-full px-3 py-2 bg-gray-50 border border-gray-200 rounded-lg outline-none text-sm focus:ring-2 focus:ring-blue-500"
                  ...
                  />
                  <div className="flex gap-3">
                    <button onClick={() => handleAction("APPROVE_TPC")} disabled={actionLoading} className="flex-1 bg-green-600 text-white py-2 rounded-lg font-semibold hover:bg-green-700 disabled:opacity-50">Approve</button>
                    <button onClick={() => handleAction("REJECT_TPC")} disabled={actionLoading} className="flex-1 bg-red-600 text-white py-2 rounded-lg font-semibold hover:bg-red-700 disabled:opacity-50">Reject</button>
                  </div>
                </div>
             )}
           </div>
